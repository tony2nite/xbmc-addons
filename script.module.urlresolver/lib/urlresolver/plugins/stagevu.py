...
class StagevuResolver(Plugin, UrlResolver, PluginSettings):
    ...
    def get_media_url(self, host, media_id):
        ...
        try:
            ...
            common.addon.log_error('Stagevu: got http error %d fetching %s' %
                                   (e.code, web_url))
            return False
        ...
        if r:
            ...
            message ='Stagevu: 1st attempt at finding the stream_url failed'
            common.addon.log_debug(message)
            r = re.search("\'flashvars\',\'file=(.+)&type=video", html)
            if r:
                stream_url = r.group(1)
            else:
                message = 'Stagevu: Giving up on finding the stream_url'
    ...
    def valid_url(self, url, host):
        return re.match('http://(www.)?stagevu.com/' +
                        'video/[0-9A-Za-z]+', url) or 'stagevu' in host
